  ...
  const onSubmit = () => {
    ...
    if (returnRequest.swap_id && onReceiveSwap) {
      ...
      return onReceiveSwap(items)
        .then(() => onDismiss())
        .then(() =>
          notification("Succès", "Retour reçu avec succès", "success")
        )
        .catch((error) =>
    ...
    <Modal handleClose={onDismiss}>
      <Modal.Body>
        <Modal.Header handleClose={onDismiss}>
          <h2 className="inter-xlarge-semibold">Recevoir un retour</h2>
        </Modal.Header>
        <Modal.Content>
          <h3 className="inter-base-semibold">Articles à recevoir</h3>
